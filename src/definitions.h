...
/**
 * \struct ForceLawParameters
 * \brief  Keeps all force law parameters for a given agent. Only one set should be active at a time.
 */
typedef struct s_force_law_params
{
...
typedef struct s_daedalus_params
{
    float default_mutation_rate;
    float panic_mutation_rate;
    float fitness;
    bool obstructed_perception;
...
typedef struct s_agent
{
    ...
    ForceLaw force_law;             // Force law currently in use by this agent
    ForceLawParameters fl_params;   // Force law parmeters for this agent

    DaedalusParameters dd_params;   // Daedalus parameters for this agent
